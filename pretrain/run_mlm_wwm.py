...
class CustomDataCollatorForWholeWordMask(DataCollatorForWholeWordMask):
    def __init__(self, tokenizer, mlm_probability=0.15, pad_to_multiple_of=None):
        super().__init__(tokenizer, mlm_probability=mlm_probability, pad_to_multiple_of=pad_to_multiple_of)

        # 读取CSV文件
        df = pd.read_csv('Depression Domain Lexicon_202003.csv', encoding='gbk')

        # 筛选抑郁倾向为1的行
        depressed_df = df[df['抑郁倾向（0非抑郁；1抑郁）'] == 1]

        # 定义一个正则表达式，用于匹配完全由汉字构成的字符串
        ...
        # 遍历筛选出来的DataFrame
        for word in depressed_df['抑郁词']:
            # 检查是否完全由汉字构成
            if hanzi_regex.match(word):
                chinese_vocab.add(word)

        # 打印结果
        print(chinese_vocab)
        self.chinese_vocab = chinese_vocab  # 定义您的中文词典
    ...
    def _whole_word_mask(self, input_tokens: List[str], max_predictions=512):
        ...
        if not isinstance(self.tokenizer, (BertTokenizer, BertTokenizerFast)):
            warnings.warn(
                "CustomDataCollatorForWholeWordMask is only suitable for BertTokenizer-like tokenizers."
            )
        ...
        for indexes in cand_indexes:
            if len(masked_lms) + len(indexes) > num_to_predict:
                continue
            word = ''.join([input_tokens[i].replace("##", "") for i in indexes])
            if word in self.chinese_vocab and not masked_lms.intersection(indexes):
                masked_lms.update(indexes)
        ...
        if len(masked_lms) < num_to_predict:
            ...
            for indexes in cand_indexes:
                if len(masked_lms) + len(indexes) > num_to_predict:
                    continue
                if not covered_indexes.intersection(indexes):
                    masked_lms.update(indexes)
                    covered_indexes.update(indexes)
...
class DataTrainingArguments:
    ...
    mlm_probability: float = field(
        default=0.2, metadata={"help": "Ratio of tokens to mask for masked language modeling loss"}
    )
...
def main():
    ...
    if data_args.train_ref_file is not None:
        print(add_chinese_references(tokenized_datasets["train"], data_args.train_ref_file)[:2])
    ...
    data_collator = CustomDataCollatorForWholeWordMask(tokenizer=tokenizer, mlm_probability=data_args.mlm_probability)
